...
def list_pokemon(session: Session, filters: FilterPokemonPublic) -> List[PokemonList]:
    ...
    if filters.nombre_parcial:
        pokemon_ids_subq = pokemon_ids_subq.where(
            func.lower(Pokemon.nombre).contains(filters.nombre_parcial.lower())
        )
    if filters.min_stat is not None:
        stats_sum = select(
            Estadistica.pokemon_id,
            (
                Estadistica.puntos_de_golpe
                + Estadistica.ataque
                + Estadistica.defensa
                + Estadistica.ataque_especial
                + Estadistica.defensa_especial
                + Estadistica.velocidad
            ).label("total"),
        ).subquery()
        pokemon_ids_subq = pokemon_ids_subq.join(
            stats_sum, Pokemon.id == stats_sum.c.pokemon_id
        )
...
def get_pokemon(id: int, session: Session) -> Optional[dict]:
    ...
    for tipo in tipos_pokemon:
        ...
        tipos_con_debilidades.append(
            {"id": tipo.id, "nombre": tipo.nombre, "debilidades": debilidades_list}
        )
    ...
    evoluciones_list = [
        {"id": evo.id, "nombre": evo.nombre, "imagen": evo.imagen}
    ...
    categoria_map = {
        c.id: c.nombre.lower() for c in session.exec(select(CategoriaMovimiento)).all()
    }
    ...
    for move, method_id in movimientos:
        ...
        nombre_efecto = session.exec(
            select(EfectoMovimiento).where(EfectoMovimiento.id == move.efecto_id)
        ).first()
        move_data = {
            "id": move.id,
            "nombre": move.nombre,
            ...
            "categoria": (
                "físico"
                if cat_nombre == "physical"
                else "especial" if cat_nombre == "special" else "estado"
            ),
