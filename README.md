[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/dcSjML4o)
...
Debemos implementar un sitio web para obtener información sobre Pokemones, sus movimientos y estadísticas así como también la posibilidad de creación de equipos de combate (similar a https://pokemondb.net/). El sitio web va constar de una parte de desarrollo backend, una base de datos para persistir la información, y una parte de desarrollo frontend. Esta primer etapa corresponde al desarrollo backend de la aplicación.
...
GET /api/pokemon/30
```
que devolverá su ID, nombre, imagen, altura, peso, tipos, stats, evoluciones y movimientos según su método de aprendizaje (huevo, máquina de tiempo, nivel).
...
El endpoint recién nombrado deberá además poder filtrar la lista de pokemones por tipo y por nombre parcial:
...
```
que devolverá su ID, nombre, su tipo (Roca, Planta, etc), categoría (Fisico, Estado, Especial, etc), potencia (power), precision (accuracy), puntos de poder o cantidad de usos posibles (pp), efecto en texto, Pokemon que lo pueden aprenden (separado según si es por huevo, máquina, o nivel)
```
{
    "id": 32,
    "nombre": "Perforador",
...
GET /api/movimientos
```
que devolverá cada movimiento con su ID, nombre, tipo (Roca, Planta, etc), categoría (Fisico, Estado, Especial, etc), potencia (power), precision (accuracy), puntos de poder (pp) y efecto en texto.


### Equipos

La escena competitiva del juego requiere que se puedan crear Equipos, pero también limita contenido hasta ciertas cantidades.

Condiciones de un equipo:
  - se permiten como máximo 6 integrantes, pero puede existir un equipo con 0 integrantes.
  - cada integrante debe referenciar a un Pokemon existente.
  - cada integrante debe tener no más de 4 movimientos, pero puede tener 0 movimientos.
  - los movimientos elegidos para cada integrante deben ser válidos para ese Pokemon.
...
GET /api/equipos/<id>
```
que devolverá su ID, nombre y la lista de integrantes con su información:
```
{
    "id": 1,
...
GET /api/equipos
```
que devolverá para una lista de equipos, donde para cada equipo detallará su ID, nombre y cantidad de integrantes.
```
[
    {
        "id": 1,
...
```
que recibirá la información del equipo a crear (nombre único) y devolverá la información del equipo creado:
```
{
    "id": 1,
    "nombre": "Equipo Rocket",
...
```
que permite cambiar el nombre del equipo.
...
Este endpoint permite agregar o borrar múltiples movimientos en una sola interacción a un mismo integrante (además de editar su apodo). Los movimientos especificados en esta operación definen el total de los movimientos que deben quedar asignados al miembro. Si hay error en alguno de los movimientos (por ejemplo, algún movimiento inválido porque no pertenece al conjunto de movimientos aprendibles por el Pokemon), debe fallar toda la operación sin haber cambiado nada.
...
POST /api/pokemon
```
La request debe poder recibir todos los detalles de Pokemon, incluyendo: nombre, altura, peso, tipos, estadísticas y movimientos según su tipo (huevo, máquina, nivel).
...
IMPORTANTE: notar que todos los campos que referencian a movimientos, tipos y otros pokemon lo hacen a a través de sus IDs y solo incluyen eso, no los objetos completos.

El Pokemon, para ser creado, debe cumplir ciertas condiciones en base a sus padres que se deben validar:
...
Estos archivos .csv se deben procesar de manera eficiente. Tener en cuenta que tienen cientos de miles de líneas y de lo contrario puede hacer que la experiencia con el sistema sea deplorable.
...
**IMPORTANTE**: se intentó sanitizar la información de los CSV por no garantizamos su fidelidad 100%. Si en algun momento encuentran una línea inválida (ya se por ejemplo un movimiento de un Pokemon que no existe, o cualquier situación similar, ignorar esa línea).
...
Se debe usar venv para setear el virtual environment. Las dependencias deben estar listadas en un archivo requirements.txt para poder ser instaladas con `pip`. Luego de tenerlo seteado, el proyecto debe poder ser corrido con el comando ```python -m run fastapi dev main.py```.
